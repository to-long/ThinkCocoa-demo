 ...
 *      → starts NEW under name `kuana-data-<NEW>`
 *   4. Workflow health-checks NEW on 127.0.0.1:<NEW>/health
 *   5. Workflow does `pm2 delete kuana-data-<OLD>` once NEW is healthy
 *
  ...
  apps: [
    {
      ...
      out_file: `/var/log/kuana-data/${PORT}-out.log`,
      error_file: `/var/log/kuana-data/${PORT}-error.log`,
      merge_logs: true,
    ...
    //
    // Logs land in /var/log/kuana-data/storage-maintenance-*.log
    // and each run prints one NDJSON summary line, easy to grep.
    {
      ...
      out_file: '/var/log/kuana-data/storage-maintenance-out.log',
      error_file: '/var/log/kuana-data/storage-maintenance-error.log',
      merge_logs: true,
      log_date_format: 'YYYY-MM-DD HH:mm:ss Z',
