...
// ── MAP VIEW ─────────────────────────────────────────────────
/**
 * Every mapped parcel in the active cooperative as one GeoJSON
 * FeatureCollection, plus the EUDR risk zones, for the full-screen map.
 *
 * One request, not one per parcel: the map draws ~1k plots at once and a
 * per-feature fetch would be a thousand round trips. Geometry is
 * simplified server-side (~11 m tolerance) — at country zoom nobody can
 * see the difference, and it roughly halves the payload.
 *
 * Parcels with no polygon fall back to their GPS pin so an unmapped-but-
 * located plot still appears; parcels with neither are skipped (they have
 * nothing to draw).
 */
...
export async function getParcelMap(activeCoopId: string): Promise<ParcelMapData> {
  const parcelFc = await db.execute(dsql`
    SELECT json_build_object(
      'type', 'FeatureCollection',
      'features', COALESCE(json_agg(json_build_object(
        'type', 'Feature',
        'properties', json_build_object(
          ...
          'farmer', NULLIF(TRIM(CONCAT(f.first_name, ' ', f.last_name)), ''),
          'areaHa', p.calculated_area_ha,
          'eudrStatus', COALESCE(e.status, 'unknown'),
          'deforestationRisk', e.deforestation_risk,
          'protectedAreaRisk', e.protected_area_risk,
          'mapped', (g.geom IS NOT NULL)
        ),
        'geometry', ST_AsGeoJSON(
          ST_SimplifyPreserveTopology(COALESCE(g.geom, g.point_geom), 0.0001)
        )::json
      )), '[]'::json)
    ) AS fc
    FROM gis.parcels p
    JOIN gis.parcel_geometries g ON g.parcel_id = p.id
    LEFT JOIN gis.eudr_status e ON e.parcel_id = p.id
    LEFT JOIN farmer.farmers f ON f.id = p.farmer_id
    WHERE p.cooperative_id = ${activeCoopId}
      AND p.deleted_at IS NULL
      AND COALESCE(g.geom, g.point_geom) IS NOT NULL
  `);

  // Zones are scoped through their source parcel so a cooperative only
  // ever sees the risk areas around its own plots.
  const zoneFc = await db.execute(dsql`
    SELECT json_build_object(
      'type', 'FeatureCollection',
      'features', COALESCE(json_agg(json_build_object(
        'type', 'Feature',
    ...
    FROM gis.risk_zones z
    JOIN gis.parcels p ON p.id = z.source_parcel_id
    WHERE p.cooperative_id = ${activeCoopId}
      AND p.deleted_at IS NULL
  `);

  return {
    parcels: (parcelFc.rows[0] as { fc?: unknown } | undefined)?.fc ?? null,
    riskZones: (zoneFc.rows[0] as { fc?: unknown } | undefined)?.fc ?? null,
  };
}
